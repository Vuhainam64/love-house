...
import { getSampleProjectById, updateSampleProject } from "../../../api";
...
function EditProject() {
  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        const response = await getSampleProjectById(id);

        if (response?.isSuccess) {
          const projectData = response.result.data;
          setTitle(projectData.sampleProject.header);
          setNumOfFloor(projectData.sampleProject.numOfFloor);
          setConstructionArea(projectData.sampleProject.constructionArea);
          setTotalArea(projectData.sampleProject.totalArea);
          setSelectedProjectType(projectData.sampleProject.projectType);
          setEstimatePrice(projectData.sampleProject.estimatePrice);
